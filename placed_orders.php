...
if ($_SERVER["REQUEST_METHOD"] && isset($_POST['delete'])) {
    ...
    // Prompt the user for confirmation
        ...
        $deleteQuery = "DELETE FROM `orders` WHERE `user_id` = $userIdToDelete";
...
if($_SERVER["REQUEST_METHOD"]){
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $id = $row['user_id'];
            ...
            echo ' <p style="margin-top: 10px; margin-bottom:10px margin-left:40px  maring-right:40px">Payment Method: ' . $method . '</p>';
            echo ' <p style="margin-top: 10px; margin-bottom:10px margin-left:40px  maring-right:40px">Address: ' . $address . '</p>';
            echo ' <p style="margin-top: 10px; margin-bottom:10px margin-left:40px  maring-right:40px"> Total Products:' . $total_products . '</p>';
            echo ' <p style="margin-top: 10px; margin-bottom:10px margin-left:40px  maring-right:40px"> Total price:' . $total_price . '</p>';
            echo ' <p style="margin-top: 10px; margin-bottom:10px margin-left:40px  maring-right:40px"> Placed on:' . $placed_on . '</p>';
            echo '<p>';
            echo '<label for="paymentStatus">Payment Status:</label>';
            echo '<select name="paymentStatus" id="paymentStatus">';
            echo '<option value="Complete" ' . ($payment_status === 'Complete' ? 'selected' : '') . '>compleat</option>';
            echo '<option value="Pending" ' . ($payment_status === 'Pending' ? 'selected' : '') . '>pending</option>';
